<?php
// import_bevetelezések.php (Javítva)
set_time_limit(300); ini_set('display_errors', 1); error_reporting(E_ALL);
...
try {
    ...
    for ($i = 1; $i < count($lines); $i++) {
        ...
        $values = str_getcsv($line); if (count($values) < 10 || empty($values[1])) { $skipped_rows++; continue; }
        $kezdes_dt = false; $befejezes_dt = false;
        foreach ($date_formats as $f) { if ($kezdes_dt===false) $kezdes_dt=DateTime::createFromFormat($f,trim($values[2]??'')); if ($befejezes_dt===false) $befejezes_dt=DateTime::createFromFormat($f,trim($values[3]??'')); }
